...
const termStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .term-root { min-height: 100vh; background: var(--bg); display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono', monospace; position: relative; overflow: hidden; padding: 32px 16px; }
  .term-root::before { content: ''; position: fixed; inset: 0; background: repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,0,0,0.07) 2px, rgba(0,0,0,0.07) 4px); pointer-events: none; z-index: 100; }
  .terminal { width: 600px; background: var(--terminal); border: 1px solid var(--border); border-radius: 8px; box-shadow: 0 24px 80px rgba(0,0,0,0.9), 0 0 0 1px rgba(57,255,20,0.04); animation: termIn 0.35s cubic-bezier(0.16,1,0.3,1) forwards; opacity: 0; transform: translateY(16px); position: relative; z-index: 10; }
  ...
  .step-item.active { color: var(--green); } .step-item.done { color: var(--green-dim); }
  ...
  .label-prefix { color: var(--green-dim); } .label-hint { margin-left: auto; font-size: 10px; color: #282828; } .label-req { color: var(--red); font-size: 10px; }
...
const friendlyStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .f-root { min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: 'DM Sans', sans-serif; position: relative; overflow: hidden; padding: 32px 24px; transition: background 0.4s; }
  .f-root.dark { background: #0d1117; }
  .f-root.light { background: #eef2f7; }

  .f-bg-blob { position: fixed; border-radius: 50%; filter: blur(90px); pointer-events: none; z-index: 0; animation: blobIn 1.4s ease forwards; opacity: 0; }
  ...
  .f-card { width: 100%; max-width: 440px; border-radius: 24px; animation: cardIn 0.5s cubic-bezier(0.16,1,0.3,1) forwards; opacity: 0; transform: translateY(22px) scale(0.98); position: relative; z-index: 10; overflow: hidden; transition: background 0.4s, box-shadow 0.4s; }
  ...
  .f-header { padding: 26px 28px 0; }

  /* Logo: image replaces emoji */
  .f-logo-row { display: flex; align-items: center; gap: 12px; margin-bottom: 22px; }
  .f-logo-icon { width: 44px; height: 44px; border-radius: 12px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; overflow: hidden; }
  ...
  .dark .f-logo-name { color: #f0f6fc; } .light .f-logo-name { color: #0f172a; }
  .f-logo-tag { font-size: 11px; font-weight: 500; letter-spacing: 0.04em; font-family: 'DM Mono', monospace; }
  .dark .f-logo-tag { color: #39ff14; } .light .f-logo-tag { color: #16a34a; }

  .f-title { font-size: 21px; font-weight: 700; letter-spacing: -0.02em; line-height: 1.15; margin-bottom: 4px; }
  .dark .f-title { color: #f0f6fc; } .light .f-title { color: #0f172a; }
  .f-subtitle { font-size: 13.5px; line-height: 1.5; }
  .dark .f-subtitle { color: #6e7681; } .light .f-subtitle { color: #6b7280; }

  /* Step indicator */
  .f-steps { display: flex; align-items: center; margin-bottom: 20px; }
  ...
  .dark .f-step-circle.pending { border-color: #30363d; background: transparent; color: #3d444d; }
  ...
  .dark .f-step-label.active { color: #39ff14; } .dark .f-step-label.done { color: #4ade80; } .dark .f-step-label.pending { color: #3d444d; }
  ...
  .dark .f-step-connector.done { background: #1a7a08; } .dark .f-step-connector.pending { background: #21262d; }
  .light .f-step-connector.done { background: #16a34a; } .light .f-step-connector.pending { background: #e2e8f0; }

  .f-body { padding: 20px 28px 28px; }
  .f-field { margin-bottom: 15px; }
  .f-field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
  .f-label { display: block; font-size: 13px; font-weight: 600; margin-bottom: 6px; letter-spacing: -0.01em; }
  .dark .f-label { color: #8b949e; } .light .f-label { color: #374151; }

  .f-input-wrap { position: relative; }
  .f-input { width: 100%; padding: 11px 16px; border-radius: 11px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 400; outline: none; transition: all 0.18s; }
  ...
  .f-eye-btn { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; padding: 4px; border-radius: 7px; font-size: 15px; transition: opacity 0.15s; opacity: 0.4; }
  .f-eye-btn:hover { opacity: 0.9; }
  .f-error-msg { font-size: 12px; color: #f85149; margin-top: 5px; font-weight: 500; }

  /* Info box */
  .f-info-box { display: flex; gap: 10px; padding: 10px 13px; border-radius: 10px; margin-bottom: 12px; font-size: 12.5px; line-height: 1.6; font-weight: 500; }
  .dark .f-info-box { background: rgba(251,191,36,0.05); border: 1px solid rgba(251,191,36,0.12); color: #a16207; }
  .light .f-info-box { background: #fffbeb; border: 1px solid #fde68a; color: #92400e; }
  .f-info-icon { font-size: 13px; flex-shrink: 0; margin-top: 2px; }

  /* Strength */
  .f-strength-wrap { margin-top: 6px; }
  .f-strength-bg { height: 4px; border-radius: 4px; overflow: hidden; margin-bottom: 5px; }
  .dark .f-strength-bg { background: #21262d; } .light .f-strength-bg { background: #e2e8f0; }
  ...
  /* Error banner */
  .f-error-banner { display: flex; align-items: flex-start; gap: 10px; padding: 12px 14px; border-radius: 11px; margin-bottom: 16px; font-size: 13px; font-weight: 500; line-height: 1.5; background: rgba(248,81,73,0.08); border: 1.5px solid rgba(248,81,73,0.22); color: #f85149; animation: shakeX 0.4s cubic-bezier(0.36,0.07,0.19,0.97); }
  @keyframes shakeX { 0%,100%{transform:translateX(0)} 20%{transform:translateX(-5px)} 40%{transform:translateX(5px)} 60%{transform:translateX(-3px)} 80%{transform:translateX(3px)} }

  /* Buttons */
  .f-btn-row { display: flex; gap: 10px; margin-top: 8px; }
  .f-btn-back { padding: 11px 18px; background: none; border-radius: 11px; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 600; cursor: pointer; transition: all 0.2s; white-space: nowrap; }
  .dark .f-btn-back { border: 1.5px solid #30363d; color: #6e7681; }
  .dark .f-btn-back:hover { border-color: #484f58; color: #8b949e; }
  .light .f-btn-back { border: 1.5px solid #e2e8f0; color: #9ca3af; }
  .light .f-btn-back:hover { border-color: #cbd5e1; color: #64748b; }
  .f-btn-next { flex: 1; padding: 12px; border: none; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 14.5px; font-weight: 700; cursor: pointer; transition: all 0.18s; }
  .dark .f-btn-next { background: #39ff14; color: #060f00; }
  .dark .f-btn-next:hover:not(:disabled) { background: #4dff28; box-shadow: 0 0 0 4px rgba(57,255,20,0.15), 0 4px 20px rgba(57,255,20,0.3); transform: translateY(-1px); }
  .light .f-btn-next { background: #16a34a; color: #fff; }
  .light .f-btn-next:hover:not(:disabled) { background: #15803d; box-shadow: 0 0 0 4px rgba(22,163,74,0.12), 0 4px 20px rgba(22,163,74,0.25); transform: translateY(-1px); }
  .f-btn-next:active:not(:disabled) { transform: translateY(0) !important; }
  .f-btn-next:disabled { opacity: 0.4; cursor: not-allowed; }
  ...
  .f-form-footer { margin-top: 18px; padding-top: 16px; text-align: center; font-size: 13.5px; }
  .dark .f-form-footer { border-top: 1px solid #21262d; color: #484f58; }
  .light .f-form-footer { border-top: 1px solid #f1f5f9; color: #9ca3af; }
  .f-form-footer button { background: none; border: none; font-family: 'DM Sans', sans-serif; font-size: 13.5px; font-weight: 700; cursor: pointer; padding: 0; margin-left: 5px; transition: color 0.15s; }
  .dark .f-form-footer button { color: #39ff14; } .dark .f-form-footer button:hover { color: #52ff2e; }
  ...
  .f-success-title { font-size: 20px; font-weight: 700; margin-bottom: 10px; letter-spacing: -0.01em; }
  .dark .f-success-title { color: #4ade80; } .light .f-success-title { color: #16a34a; }
  .f-success-sub { font-size: 14px; line-height: 1.7; }
  .dark .f-success-sub { color: #6e7681; } .light .f-success-sub { color: #6b7280; }
...
function FriendlyRegister({ onGoToLogin, theme, onToggleTheme, onToggleMode, onRegister }) {
  ...
  const stepState = (n) => n < step ? "done" : n === step ? "active" : "pending";
      ...
      <div className={`f-root ${theme}`}>
        ...
        <div className="f-card">
          ...
          <div className="f-success">
            ...
            <div style={{ marginTop: 20, fontSize: 12, opacity: 0.4, fontFamily: "'DM Mono', monospace" }}>
        ...
        <div className="f-body">
          ...
          <div className="f-steps">
            <div className="f-step">
              <div className={`f-step-circle ${stepState(1)}`}>{step > 1 ? "✓" : "1"}</div>
          ...
          {banner && <div className="f-error-banner"><span>⚠</span><span>{banner}</span></div>}
            ...
            <>
              <div className="f-field-row">
                <div className="f-field">
                  <label className="f-label">First name <span style={{color:"#f85149"}}>*</span></label>
                  <input
                    ref={firstRef}
                ...
                <div className="f-field">
                  <label className="f-label">Last name <span style={{color:"#f85149"}}>*</span></label>
                  <input
              ...
              <div className="f-field">
                <label className="f-label">Student ID <span style={{color:"#f85149"}}>*</span></label>
                <div className="f-info-box">
                  <span className="f-info-icon">ℹ️</span>
                  <span>Your Student ID verifies your role as sit-in monitoring staff. Enter it exactly as shown on your school-issued ID.</span>
                </div>
                <input
          ...
          {step === 2 && (
            <>
              <div className="f-field">
                <label className="f-label">Email address <span style={{color:"#f85149"}}>*</span></label>
                <input
                  ref={emailRef}
              ...
              <div className="f-field">
                <label className="f-label">Password <span style={{color:"#f85149"}}>*</span></label>
                ...
                {password && (
                  <div className="f-strength-wrap">
                    ...
                    <span className="f-strength-label" style={{ color: strength.color }}>Strength: {strength.label}</span>
                  </div>
                )}
              </div>
...
function TerminalRegister({ onRegister, onGoToLogin, onToggleMode }) {
  ...
  const validateStep1 = () => {
    ...
    if (!lastName.trim()) e.lastName = "Last name is required.";
    if (!studentId.trim()) e.studentId = "Student ID is required.";
    else if (!/^\d{7,12}$/.test(studentId.replace(/-/g, ""))) e.studentId = "Enter a valid Student ID (7–12 digits).";
    setErrors(e); return Object.keys(e).length === 0;
  };
  const validateStep2 = () => {
    const e = {};
